...
@tags = Question.tag_counts_on(:tags).limit(7).order('count desc')
klass = Question
klass = klass.tagged_with(@tag) if (@tag = params[:tag]).present?
@questions = klass.where(:state => '4').search(params[:search]).order('accepted desc').paginate(:page => params[:page], :per_page => 10)
...
# Εάν μια ερώτηση έχει ήδη γίνει αποδεκτή ο φοιτητής δεν επιτρέπεται να κάνει αλλαγές στον τίτλο
if @question.state > 2
